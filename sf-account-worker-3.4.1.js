...
async function ensureDatabase(db) {
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS pz_cw_blueprints_v1 (
      id TEXT PRIMARY KEY,
...
export class PZCrystalWarRoom {
  ...
  onMessage(userId, raw){
    ...
    if(m.type==="scene"){
      ...
      if(activity==="battle"){p.area=Math.max(1,Math.floor(Number(m.area)||Number(p.area)||1));p.route=["center","upper","lower"].includes(String(m.route))?String(m.route):String(p.route||"center");}
    ...
    if(m.type==="initialize"){
      p.area=Math.max(1,Math.floor(Number(m.area)||Number(p.area)||1));p.route=["center","upper","lower"].includes(String(m.route))?String(m.route):"center";
      let enemyIndex=0;for(const e of Array.isArray(m.enemies)?m.enemies.slice(0,32):[]){const id=String(e.id||"").slice(0,48);if(id&&!this.world.enemies[id])this.world.enemies[id]={id,type:String(e.type||"normal"),area:p.area,route:p.route,x:Number(e.x)||700,y:Number(e.y)||360,hp:Math.max(0,Number(e.hp)||1),maxHp:Math.max(1,Number(e.maxHp)||1),alive:e.alive!==false,vx:0,vy:0,state:"idle",attackAt:Date.now()+180+(enemyIndex++%6)*120};}
      for(const n of Array.isArray(m.resources)?m.resources.slice(0,64):[]){const id=String(n.id||"").slice(0,48);if(id&&!this.world.resources[id])this.world.resources[id]={...n,id,area:p.area,route:p.route,remaining:Math.max(0,Number(n.remaining)||0)};}
  ...
  handleRouteRequest(userId,m){
    ...
    const fromArea=Math.max(1,Math.floor(Number(p.area)||1)),fromRoute=String(p.route||"center"),targetArea=Math.max(1,Math.floor(Number(m.area)||fromArea)),targetRoute=["center","upper","lower"].includes(String(m.route))?String(m.route):"center";
    const localEnemies=Object.values(this.world.enemies).filter(e=>Number(e.area||fromArea)===fromArea&&String(e.route||fromRoute)===fromRoute),cleared=localEnemies.length===0||localEnemies.every(e=>!e.alive);
    ...
    this.world.routeRevision=Number(this.world.routeRevision||0)+1;
    ...
    p.area=targetArea;p.route=targetRoute;p.x=spawn.x;p.y=spawn.y;p.vx=0;p.vy=0;p.moveX=0;p.moveY=0;
    this.send(this.clients.get(userId)?.socket,{type:"route_changed",eventId:"route:"+this.world.routeRevision,serverTime:Date.now(),area:targetArea,route:targetRoute,routeRevision:this.world.routeRevision,spawn,restored:false,world:this.fullState()});
    this.broadcast({type:"player_scene",serverTime:Date.now(),player:this.publicPlayer(p)},userId);
  }

  handleAction(userId,m){
    const client=this.clients.get(userId),p=this.world.players[userId],sequence=Math.floor(Number(m.sequence||0));if(!client||!p||p.activity!=="battle"||sequence<=client.meta.lastActionSequence)return;client.meta.lastActionSequence=sequence;
  ...
  handleHit(userId,m){
    const client=this.clients.get(userId),p=this.world.players[userId],sequence=Math.floor(Number(m.sequence||0));if(!client||!p||p.activity!=="battle"||sequence<=client.meta.lastHitSequence)return;client.meta.lastHitSequence=sequence;
    const eventId=String(m.eventId||userId+":hit:"+sequence);if(this.seenEvent(eventId))return;
    const enemy=this.world.enemies[String(m.enemyId||"")],sourceKind=String(m.sourceKind||"basic");if(!enemy||!enemy.alive||Number(enemy.area||p.area)!==p.area||String(enemy.route||p.route)!==String(p.route))return;
